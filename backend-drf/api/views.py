...
class StockPredictionAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # Ftech The Data From YTfinance
            now= datetime.now()
            start=datetime(now.year-10,now.month,now.day)
            end=now
            df=yf.download(ticker,start,end)
            if df.empty:
                return Response({"message":"No Data Found for the Given Ticker"}, status=status.HTTP_404_NOT_FOUND)
            ...
            # Scaling with Data between the 0 and 1
            ...
            # Revert the Scale Prices to Original Price 
